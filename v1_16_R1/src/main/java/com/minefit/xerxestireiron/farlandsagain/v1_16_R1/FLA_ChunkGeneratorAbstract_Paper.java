...
public final class FLA_ChunkGeneratorAbstract_Paper extends ChunkGenerator {

    public static final Codec<FLA_ChunkGeneratorAbstract_Paper> d = RecordCodecBuilder.create((instance) -> {
        ...
        })).apply(instance, instance.stable(FLA_ChunkGeneratorAbstract_Paper::new));
    });
    ...
    private final int o;
    private final int p;
    // Several of these are normally final, had to remove that because try/catch
    ...
    public FLA_ChunkGeneratorAbstract_Paper(WorldChunkManager worldchunkmanager, long i, GeneratorSettingBase generatorsettingbase) {
        this(worldchunkmanager, worldchunkmanager, i, generatorsettingbase, null, null, 4);
    }

    public FLA_ChunkGeneratorAbstract_Paper(WorldChunkManager worldchunkmanager, long i, GeneratorSettingBase generatorsettingbase, ConfigValues configValues, ChunkGenerator originalGenerator, int divisor) {
        this(worldchunkmanager, worldchunkmanager, i, generatorsettingbase, configValues, originalGenerator, divisor);
    }

    private FLA_ChunkGeneratorAbstract_Paper(WorldChunkManager worldchunkmanager, WorldChunkManager worldchunkmanager1, long i, GeneratorSettingBase generatorsettingbase, ConfigValues configValues, ChunkGenerator originalGenerator, int divisor) {
        ...
        try {
            Field e = ReflectionHelper.getField(originalGenerator.getClass(), "e", true);
            e.setAccessible(true);
            this.e = (SeededRandom) e.get(originalGenerator);
            ...
            Field r = ReflectionHelper.getField(originalGenerator.getClass(), "r", true);
            r.setAccessible(true);
            this.r = (NoiseGeneratorOctaves) r.get(originalGenerator);
            Field s = ReflectionHelper.getField(originalGenerator.getClass(), "s", true);
            s.setAccessible(true);
            this.s = (NoiseGeneratorOctaves) s.get(originalGenerator);
            Field t = ReflectionHelper.getField(originalGenerator.getClass(), "t", true);
            t.setAccessible(true);
            this.t = (NoiseGenerator) t.get(originalGenerator);
            Field u = ReflectionHelper.getField(originalGenerator.getClass(), "u", true);
            u.setAccessible(true);
            this.u = (NoiseGeneratorOctaves) u.get(originalGenerator);

            if (noisesettings.k()) {
                Field v = ReflectionHelper.getField(originalGenerator.getClass(), "v", true);
                v.setAccessible(true);
                this.v = (NoiseGenerator3Handler) v.get(originalGenerator);
            } else {
                this.v = null;
            }
    ...
    protected Codec<? extends ChunkGenerator> a() {
        return FLA_ChunkGeneratorAbstract_Paper.d;
    }
    ...
    private double a(int i, int j, int k, double d0, double d1, double d2, double d3) {
        ...
        for (int l = 0; l < 16; ++l) {
            ...
            if (noisegeneratorperlin != null) {
                d4 += noisegeneratorperlin.a(d8 * iMultiX, d9, d10 * kMultiZ, d11, (double) j * d11) / d7;
            }

            NoiseGeneratorPerlin noisegeneratorperlin1 = this.r.a(l);

            if (noisegeneratorperlin1 != null) {
                d5 += noisegeneratorperlin1.a(d8 * iMultiX, d9, d10 * kMultiZ, d11, (double) j * d11) / d7;
            }
        ...
        if (this.v != null) {
            ...
            for (int l = -2; l <= 2; ++l) {
                for (int i1 = -2; i1 <= 2; ++i1) {
                    ...
                    float f9 = f8 * FLA_ChunkGeneratorAbstract_Paper.j[l + 2 + (i1 + 2) * 5] / (f6 + 2.0F);
    ...
    protected IBlockData a(double d0, int i) {
        ...
        if (d0 > 0.0D) {
            ...
            iblockdata = FLA_ChunkGeneratorAbstract_Paper.k;
        }
    ...
    private void a(IChunkAccess ichunkaccess, Random random) {
        ...
        if (flag1 || flag2) {
            ...
            while (iterator.hasNext()) {
                ...
                // FarLandsAgain: Modified for Paper's flat bedrock settings
                if (flag1) {
                    for (i1 = 0; i1 < 5; ++i1) {
                        if (i1 <= (this.configValues.paperSpigot.generateFlatBedrock ? l : random.nextInt(5))) {
                ...
                if (flag2) {
                    for (i1 = 4; i1 >= 0; --i1) {
                        if (i1 <= (this.configValues.paperSpigot.generateFlatBedrock ? k : random.nextInt(5))) {
                    ...
                    for (int i2 = this.l - 1; i2 >= 0; --i2) {
                        ...
                        for (int i3 = 0; i3 < this.m; ++i3) {
                            ...
                            for (int l3 = 0; l3 < this.m; ++l3) {
                                ...
                                if (iblockdata != FLA_ChunkGeneratorAbstract_Paper.k) {
                                    if (iblockdata.f() != 0) {
    ...
    private static double a(int i, int j, int k) {
        ...
        return l >= 0 && l < 24 ? (i1 >= 0 && i1 < 24 ? (j1 >= 0 && j1 < 24 ? (double) FLA_ChunkGeneratorAbstract_Paper.i[j1 * 24 * 24 + l * 24 + i1] : 0.0D) : 0.0D) : 0.0D;
    }
    ...
    public void addMobs(RegionLimitedWorldAccess regionlimitedworldaccess) {
        // FarLandsAgain: Whatever this is, it's protected so we gotta work around
        //if (!this.h.h()) {
        if (!this.mobH) {
