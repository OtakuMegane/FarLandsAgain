...
public class Beardifier {
    ...
    private static final float[] BEARD_KERNEL = (float[]) SystemUtils.a(new float[13824], (afloat) -> {
        for (int i = 0; i < 24; ++i) {
            for (int j = 0; j < 24; ++j) {
                for (int k = 0; k < 24; ++k) {
    ...
    protected Beardifier(StructureManager structuremanager, IChunkAccess ichunkaccess) {
        ...
        Iterator iterator = StructureGenerator.t.iterator(); // NOISE_AFFECTING_FEATURES -> T
                ...
                while (iterator1.hasNext()) {
                    ...
                    if (structurepiece.a(chunkcoordintpair, 12)) {
                        if (structurepiece instanceof WorldGenFeaturePillagerOutpostPoolPiece) {
                            ...
                            if (worldgenfeaturedefinedstructurepooltemplate_matching == WorldGenFeatureDefinedStructurePoolTemplate.Matching.b) { // RIGID -> b
    ...
    protected double a(int i, int j, int k) {
        ...
        while (this.pieceIterator.hasNext()) {
            ...
            if (noiseeffect == NoiseEffect.b) { // BURY -> b
                d0 += b(l, i1, j1);
            } else if (noiseeffect == NoiseEffect.c) { // BEARD -> c
                d0 += c(l, i1, j1) * 0.8D;
            }
        }
