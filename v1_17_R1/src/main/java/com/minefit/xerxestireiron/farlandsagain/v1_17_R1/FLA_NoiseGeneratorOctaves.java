...
import org.bukkit.craftbukkit.libs.it.unimi.dsi.fastutil.ints.IntBidirectionalIterator;
import org.bukkit.craftbukkit.libs.it.unimi.dsi.fastutil.ints.IntRBTreeSet;
import org.bukkit.craftbukkit.libs.it.unimi.dsi.fastutil.ints.IntSortedSet;
...
public class FLA_NoiseGeneratorOctaves implements NoiseGenerator {
    ...
    public FLA_NoiseGeneratorOctaves(RandomSource randomsource, IntStream intstream) {
        this(randomsource, (List) intstream.boxed().collect(ImmutableList.toImmutableList()));
    }

    public FLA_NoiseGeneratorOctaves(RandomSource randomsource, List<Integer> list) {
        this(randomsource, (IntSortedSet) (new IntRBTreeSet(list)));
    }

    public static FLA_NoiseGeneratorOctaves a(RandomSource randomsource, int i, double... adouble) {
        return a(randomsource, i, (DoubleList) (new DoubleArrayList(adouble)));
    }

    public static FLA_NoiseGeneratorOctaves a(RandomSource randomsource, int i, DoubleList doublelist) {
        return new FLA_NoiseGeneratorOctaves(randomsource, Pair.of(i, doublelist));
    }
    ...
    private FLA_NoiseGeneratorOctaves(RandomSource randomsource, IntSortedSet intsortedset) {
        this(randomsource, intsortedset, SeededRandom::new);
    }

    private FLA_NoiseGeneratorOctaves(RandomSource randomsource, IntSortedSet intsortedset, LongFunction<RandomSource> longfunction) {
        this(randomsource, a(intsortedset), longfunction);
    }

    protected FLA_NoiseGeneratorOctaves(RandomSource randomsource, Pair<Integer, DoubleList> pair) {
        this(randomsource, pair, SeededRandom::new);
    }

    protected FLA_NoiseGeneratorOctaves(RandomSource randomsource, Pair<Integer, DoubleList> pair, LongFunction<RandomSource> longfunction) {
    ...
    public double a(double d0, double d1, double d2, double d3, double d4, boolean flag) {
        ...
        for (int i = 0; i < this.noiseLevels.length; ++i) {
            ...
            if (noisegeneratorperlin != null) {
                double d8 = noisegeneratorperlin.a(a(d0 * d6), flag ? -noisegeneratorperlin.b : a(d1 * d6), a(d2 * d6), d3 * d6, d4 * d6); // NoiseGeneratorPerlin yo -> b
